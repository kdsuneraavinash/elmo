...
def read_corpus(path, max_chars=None, max_sent_len=20):
    ...
    :return:
    """
    logging.info("Reading corpus {}".format(path))
...
def create_batches(x, batch_size, word2id, char2id, config, perm=None, shuffle=True, sort=True, use_cuda=False):
    ...
    for i in range(nbatch):
        ...
        logging.info("{} out of {} batches crated".format(i, nbatch))
...
class Model(nn.Module):
    def __init__(self, config, word_emb_layer, char_emb_layer, n_class, use_cuda=False):
        ...
        if config['classifier']['name'].lower() == 'softmax':
            ...
            self.classify_layer = SampledSoftmaxLayer(
                self.output_dim, n_class, config['classifier']['n_samples'], use_cuda)
    ...
    def forward(self, word_inp, chars_inp, mask_package):
        ...
        if self.training and classifier_name == 'cnn_softmax' or classifier_name == 'sampled_softmax':
            self.classify_layer.update_negative_samples(
                word_inp, chars_inp, mask_package[0])
            self.classify_layer.update_embedding_matrix()

        token_embedding = self.token_embedder(
            word_inp, chars_inp, (mask_package[0].size(0), mask_package[0].size(1)))
        token_embedding = F.dropout(
            token_embedding, self.config['dropout'], self.training)

        encoder_name = self.config['encoder']['name'].lower()
        if encoder_name == 'elmo':
            mask = Variable(mask_package[0].cuda()).cuda(
        ...
        encoder_output = F.dropout(
            encoder_output, self.config['dropout'], self.training)
        ...
        mask1 = Variable(mask_package[1].cuda()).cuda(
        ) if self.use_cuda else Variable(mask_package[1])
        mask2 = Variable(mask_package[2].cuda()).cuda(
        ) if self.use_cuda else Variable(mask_package[2])
    ...
    def save_model(self, path, save_classify_layer):
        ...
        if save_classify_layer:
            torch.save(self.classify_layer.state_dict(),
                       os.path.join(path, 'classifier.pkl'))

    def load_model(self, path):
        self.token_embedder.load_state_dict(
            torch.load(os.path.join(path, 'token_embedder.pkl')))
        self.encoder.load_state_dict(
            torch.load(os.path.join(path, 'encoder.pkl')))
        self.classify_layer.load_state_dict(
            torch.load(os.path.join(path, 'classifier.pkl')))
...
def train():
    ...
    cmd.add_argument('--config_path', required=True,
    ...
    cmd.add_argument("--batch_size", "--batch", type=int,
    ...
    cmd.add_argument('--max_sent_len', type=int, default=20,
                     help='maximum sentence length.')

    cmd.add_argument('--min_count', type=int, default=5,
                     help='minimum word count.')

    cmd.add_argument('--max_vocab_size', type=int,
                     default=150000, help='maximum vocabulary size.')

    cmd.add_argument('--save_classify_layer', default=False, action='store_true',
                     help="whether to save the classify layer")

    cmd.add_argument('--valid_size', type=int, default=0,
                     help="size of validation dataset when there's no valid.")
    cmd.add_argument('--eval_steps', required=False, type=int,
    ...
    logging.info("Using cuda: " + str(use_cuda))
    ...
    if token_embedder_name == 'cnn':
        train_data = read_corpus(
            opt.train_path, token_embedder_max_chars, opt.max_sent_len)
    ...
    if opt.valid_path is not None:
        if token_embedder_name == 'cnn':
            valid_data = read_corpus(
                opt.valid_path, token_embedder_max_chars, opt.max_sent_len)
        elif token_embedder_name == 'lstm':
    ...
    if opt.test_path is not None:
        if token_embedder_name == 'cnn':
            test_data = read_corpus(
                opt.test_path, token_embedder_max_chars, opt.max_sent_len)
        elif token_embedder_name == 'lstm':
    ...
    if config['token_embedder']['word_dim'] > 0:
        word_emb_layer = EmbeddingLayer(
            config['token_embedder']['word_dim'], word_lexicon, fix_emb=False, embs=embs)
        logging.info('Word embedding size: {0}'.format(
            len(word_emb_layer.word2id)))
    else:
        word_emb_layer = None
    ...
    if config['token_embedder']['char_dim'] > 0:
        ...
        char_emb_layer = EmbeddingLayer(
            config['token_embedder']['char_dim'], char_lexicon, fix_emb=False)
        logging.info('Char embedding size: {0}'.format(
            len(char_emb_layer.word2id)))
    else:
        char_lexicon = None
        char_emb_layer = None
    ...
    def need_grad(x): return x.requires_grad
    if opt.optimizer.lower() == 'adam':
        optimizer = optim.Adam(
            filter(need_grad, model.parameters()), lr=opt.lr)
    ...
    json.dump(vars(opt), codecs.open(os.path.join(
        opt.model, 'config.json'), 'w', encoding='utf-8'))
...
def test():
    ...
    args2 = dict2namedtuple(json.load(codecs.open(
        os.path.join(args.model, 'config.json'), 'r', encoding='utf-8')))
    ...
    model = Model(config, word_emb_layer, char_emb_layer,
                  len(word_lexicon), use_cuda)
    ...
    if config['token_embedder']['name'].lower() == 'cnn':
        test = read_corpus(
            args.input, config['token_embedder']['max_characters_per_token'], max_sent_len=10000)
    elif config['token_embedder']['name'].lower() == 'lstm':
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == 'train':
        ...
        print('Usage: {0} [train|test] [options]'.format(
